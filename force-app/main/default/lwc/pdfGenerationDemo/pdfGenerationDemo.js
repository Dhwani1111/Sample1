...
export default class PdfGenerationDemo extends LightningElement {
    @api recordId
    // imageUrl = 'https://i.picsum.photos/id/537/200/300.jpg?hmac=LG3kZs5AdrMmsgeVOdrfP0C5KT3WmP-q5TauEZdR4vk'
    invoiceData={
        invoiceNo:'123',
    ...
    pdfHandler(){
        ...
        generatePDF({ recordId:this.recordId, htmlData:content.outerHTML}).then(result=>{
            ...
            window.open(`https://empathetic-moose-t7givn-dev-ed--c.documentforce.com/servlet/servlet.FileDownload?file=${result.Id}`)
        }).catch(error=>{
            console.error(error)
        })
    }
